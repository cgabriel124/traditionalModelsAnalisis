...
for userIdx = 1:length(userFolders)
    ...
    fprintf('\n🟦 Procesando usuario: %s (%d de %d)\n', userFolder, userIdx, length(userFolders));
    ...
    %Esta linea corrige el problema de los nombres de los usuarios
    newUserData.userInfo.name = userFolder;
    ...
    %if isfield(userDataContent.gestures, 'sync') && isfield(userDataContent.gestures.sync, 'data')
    if true
        if ~isfile('burnedSync.mat')
            % Primera iteración: Procesar y guardar sync data
            syncInstance = userDataContent.gestures.sync.data{1}; % Obtener el primer elemento de sync.data
            newUserData.synchronizationGesture.samples.idx_1 = struct();

            % Añadir el parámetro "startPointforGestureExecution"
            newUserData.synchronizationGesture.samples.idx_1.startPointforGestureExecution = syncInstance.pointGestureBegins;
            ...
            % Copiar sync 5 veces para normalización
            copySync = newUserData.synchronizationGesture.samples.idx_1;
            newUserData.synchronizationGesture.samples.idx_2 = copySync;
            newUserData.synchronizationGesture.samples.idx_3 = copySync;
            newUserData.synchronizationGesture.samples.idx_4 = copySync;
            ...
            % Guardar los datos procesados en burnedSync.mat
            save('burnedSync.mat', 'newUserData');
            fprintf('Data sync guardada en burnedSync.mat.\n');
        else
            % Iteraciones posteriores: Cargar datos desde burnedSync.mat
            loadedData = load('burnedSync.mat', 'newUserData');
            newUserData.synchronizationGesture = loadedData.newUserData.synchronizationGesture;
            fprintf('Data sync cargada desde burnedSync.mat.\n');
        end
    ...
    % Initialize the 'trainingSamples' field
    ...
    gestureTypes = {"relax", "fist", "open", "pinch", "waveIn", "waveOut"}; % Define gesture types

    for g = 1:length(gestureTypes)
        gestureName = gestureTypes{g}; % Current gesture name
        fprintf('   🔹 Gesto actual: %s\n', gestureName);
