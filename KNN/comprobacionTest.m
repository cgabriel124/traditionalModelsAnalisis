...
%usuarios.nombres = [ "aauser3","aauser6","aauser4","aauser3"];
gestosFinales=[];
...
for kUsuarios=1:length(usuarios.nombres)
    ...
    %for kGesture= 1:numGestures-1 %sin no gesture en la matriz de conf
        ...
        histogramaGestosPerUser = [(0:numGestures-1)' zeros(numGestures,1)];

        gestosFinales=[gestosFinales;nameGestures{kGesture}]
        ...
        for kRutina=1:numRepTest
            ...
            %disp("XXXXXXXXXXXX")
            %disp(logResultadosFiltrados)
            %disp("XXXXXXXXXXXX")
            %disp(logResultadosCrudos)
            %disp("XXXXXXXXXXXX")
    ...
    disp(valorRealPruebaPerUser);
    disp("KKKKKKKKKKKKKKK")
