...
%% Recognition loop
%fprintf('Se generarán %d ventanas para esta señal.\n', numExecutionsTimer);
...
while kExecutions-1<numExecutionsTimer
    ...
    % Corregir clase: si es "noGesto" (índice 6), la clase debe ser 0
    if strcmp(gestoString, 'noGesto')
    claseReal = 0;
else
    claseReal = find(strcmp(nameGestures, gestoString)); % obtiene el índice real
end



    % >>> Imprimir el gesto predicho en cada ventana
    %fprintf('Ventana %d: Gesto KNN = %s, Probabilidad = %.2f\n', kExecutions, gestoString, probGestureKNN);
    ...
    if gesto==0 || strcmp(gestoString,'noGesto')
        ...
        resultadosGestoVector(kExecutions) = claseReal;
%%%%%%%%%%%%%
    ...
    % Imprimir gesto filtrado real por ventana
    if resultadosGestoVector(kExecutions) ~= 0
        %fprintf('>> Ventana %d: Gesto FILTRADO = %s\n', ... kExecutions, nameGestures{resultadosGestoVector(kExecutions)});
    else
        %fprintf('>> Ventana %d: Gesto FILTRADO = noGesto\n', kExecutions);
    end
...
histogramaGestos = [(0:numGestures-1)', zeros(numGestures,1)];

for d = 0:numGestures-1
