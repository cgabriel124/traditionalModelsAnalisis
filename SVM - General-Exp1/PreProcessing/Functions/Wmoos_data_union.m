        ...
        for j = 1:size_checked
            ...
            if (strcmpi(folders(i).name, users_good_checked{j,1}))
                ...
                for wm=1:WM_gestures
                    ...
                    [emg,p, gesture_info]=WM_Extraction_mod(nameUser_,wm_gesture,e_,ventana,rotationMode);


                     % Acumulamos la información de los gestos en gesture_info_union
                    if isempty(gesture_info_union)
                        gesture_info_union = gesture_info;
                    else
                        gesture_info_union = [gesture_info_union; gesture_info]; % Concatenamos la nueva info
                    end
                    
                    assignin("base", "emgxd", emg);
                    assignin("base", "p", p);
                    assignin("base", "gesture_info_union", gesture_info_union);
                    save('trainDataLabels.mat', 'gesture_info_union');
