...
carpetaBase = fullfile(currentFolder, 'results', 'ann');
patronArchivos = 'ResultadosEstructurados_*.mat';
disp(carpetaBase);

archivos = dir(fullfile(carpetaBase, patronArchivos));
...
for i = 1:length(archivos)
    rutaCompleta = fullfile(archivos(i).folder, archivos(i).name);
    data = load(rutaCompleta);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%GRAFICA INDIVIDUAL%%%%%%%%
% --- Graficar métricas individuales por usuario en una figura 2x2 ---
users = fieldnames(userData);
metricNames = {'Classification', 'Recognition', 'Overlap', 'Time (s)'};
metricFields = {'classification', 'recognition', 'overlapping', 'meanTimeGesture'};  % Asegúrate que este campo exista

for u = 1:length(users)
    user = users{u};
    figure('Name', ['TEST metrics - ', user], 'NumberTitle', 'off', 'Position', [100, 100, 800, 600]);
    sgtitle(['Evolución Testing - Usuario: ', user]);

    for i = 1:4
        subplot(2, 2, i);
        if i == 4  % Tiempo
            if isfield(userData.(user), 'meanTimeGesture')
                values = userData.(user).meanTimeGesture;
            else
                values = nan(1, numMonths);
            end
        else
            values = userData.(user).(metricFields{i});
        end

        plot(1:numMonths, values, '-o', 'LineWidth', 2);
        xlabel('Mes');
        ylabel(metricNames{i});
        title([metricNames{i}, ' (Testing)']);
        xticks(1:numMonths);
